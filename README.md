# Oil & Gas Companies Performance In Indonesia Infographic
...
valuation %>% 
  ...
  geom_text(aes(x = pb + 1, y = reorder(company, pb), label = paste0(round(pb, 1), "%")),
            color = txt_col) +
...
valuation %>% 
  ...
  geom_text(aes(x = pe + 1, y = reorder(company, pe), label = paste0(round(pe, 1), "%")),
            color = txt_col) +
  scale_fill_manual(values = colors) +
...
valuation %>% 
  ...
  geom_textbox(x = 6, y = stock_fullnames[4],
            ...
            fill = txt_col, color = bg_col) +
  geom_text(aes(x = ps + 0.5, y = reorder(company, ps), label = paste0(round(ps, 1), "%")),
            color = txt_col) +
  scale_fill_manual(values = colors) +
